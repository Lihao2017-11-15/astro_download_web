---
title: Firefox
description: “Firefox是一款引擎反应更快,内存占用更少的浏览器，支持数以千计的浏览器扩展，可为浏览器设置不同的主题，Firefox 可以自动帮您导入 Chrome 上的书签、自动填写的表单、密码以及偏好设置等。”
version: V 75.0
category: 浏览器
tags: ["浏览器"]
downloadUrl: https://download-ssl.firefox.com.cn/releases/firefox/75.0/zh-CN/Firefox-latest.dmg
publishDate: 2020-04-23
updateDate: 2020-04-23
author: 烤鸡架解说
featured: false
---

# Firefox

## 软件简介

“Firefox是一款引擎反应更快,内存占用更少的浏览器，支持数以千计的浏览器扩展，可为浏览器设置不同的主题，Firefox 可以自动帮您导入 Chrome 上的书签、自动填写的表单、密码以及偏好设置等。”

## 详细介绍

概述

Firefox是一款引擎反应更快,内存占用更少的浏览器，支持数以千计的浏览器扩展，可为浏览器设置不同的主题，Firefox 可以自动帮您导入 Chrome 上的书签、自动填写的表单、密码以及偏好设置等。

功能简介

· 账户同步：支持同步书签、自动填写的表单、密码以及偏好设置等
· 浏览器拓展：支持 LastPass、Evernote 等数以千计的扩展
· 官方内置不同的浏览器主题，可自由定制
· 隐私保护：Firefox 会在您上网时拦截追踪器，也能在浏览结束后自动清除浏览记录

## 推荐语

界面清爽，多端同步；操作流畅，体验一级！

## 下载信息

- **版本**: V 75.0
- **下载链接**: [https://download-ssl.firefox.com.cn/releases/firefox/75.0/zh-CN/Firefox-latest.dmg](https://download-ssl.firefox.com.cn/releases/firefox/75.0/zh-CN/Firefox-latest.dmg)
- **发布日期**: 2020-04-23
- **更新日期**: 2020-04-23
- **推荐人**: 烤鸡架解说
- **分类**: 浏览器
